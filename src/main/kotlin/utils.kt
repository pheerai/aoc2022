...
fun <T,U,V,Z: Sequence<Pair<T,U>>> Z.mapFirsts(action: (T) -> V): Sequence<Pair<V,U>> = this.map { it.mapFirst(action) }
fun <T,U,V,Z: Sequence<Pair<T,U>>> Z.mapSeconds(action: (U) -> V): Sequence<Pair<T,V>> = this.map{ it.mapSecond(action) }
